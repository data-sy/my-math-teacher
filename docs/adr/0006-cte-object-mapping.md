# ADR 0006: MySQL CTE 객체 반환 메서드의 도메인 매핑 — `concepts JOIN chapters`, conceptSection 매핑 생략
...
본 ADR은 위 3개 자료와 시드/스키마/단일 필드 accessor를 모두 grep한 결과(보조 자료: ADR 0006 evidence)에 따라 spec-02 진입 전 객체 매핑 정책을 단일 결정으로 고정한다.
...
(ADR 0003 backward 방향 단일안 + 본 ADR JOIN 패턴을 결합한 형태.)
...
- 결정 근거 raw 데이터: `docs/adr/0006-cte-object-mapping-evidence.md`
...
- 관련 ADR: ADR 0003 (knowledge_space 방향성 — backward 단일안), ADR 0002 §2 (테스트 프로파일)
