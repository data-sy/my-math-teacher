...
// mysqlConceptRepository 필드 타입은 Optional<MysqlConceptRepository>
// (플래그 false 시 bean 미존재 — spec-02 Task 3.1 참조).
...
규약:
- 키 prefix 3종 (반환 타입별 분리, spec-02 Task 2.1과 정합):
  - `graph:prerequisites:ids:<conceptId>:<depth>` — `findPrerequisiteConceptIds` 결과 (ID 반환)
