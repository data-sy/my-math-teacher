...
피드백 검토에서 다음이 명확해졌다:
- spec-01은 ID 반환 메서드(`findPrerequisitesWithDepth`)만 도입하므로 객체 매핑 결정은 spec-01 진입 차단 사유가 아님
...
spec-02에서 도입할 객체 반환 메서드(`findPrerequisiteConcepts(int conceptId, int maxDepth)` 등)는 **`concepts JOIN chapters` JOIN 패턴**으로 `Concept` 객체를 매핑하며, spec-01 결정에 따라 `JdbcTemplateConceptRepository`에 누적된다(M1의 `MysqlConceptRepository` 인터페이스/Stub은 spec-01에서 삭제됨). `JdbcTemplateConceptRepository.findOneByConceptId`의 prior art를 그대로 확장한다.
...
- **`conceptSection`은 매핑 생략** (RowMapper에서 미설정 → null)
  ...
  - 단, 그래프 경로는 `conceptChapterId`/`conceptAchievementId`도 매핑하므로 이 두 필드만 단일 경로 대비 새 매핑이 추가됨 (둘 다 프론트 미바인딩이므로 사용자 영향 0)
