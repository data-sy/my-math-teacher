...
> ⚠️ `deployfront.sh` 만은 소진되지 않았다 — 프론트를 새로 배포할 때마다 쓴다.
> **현행본 = [`../../scripts/deploy-front.sh`](../../scripts/deploy-front.sh)** (2.0.2 기준·번들 검증 문자열을 `MARKER` 로 뽑음).
> 여기 있는 사본은 2.0.1 당시의 기록이다 — 실행하지 말 것.
