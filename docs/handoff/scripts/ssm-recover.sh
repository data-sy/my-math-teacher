...
#
#   [갱신 2026-08-31] 재발 방지가 적용됐다 — 필터는 "al2023-ami-2023.*-x86_64" 로 조여졌고
#   (위 본문의 "al2023-ami-*-x86_64" 는 당시 기록), user_data 가 에이전트를 설치하며,
#   lifecycle ignore_changes 로 우발 교체도 막혔다. 즉 **새 인스턴스는 이 결손을 갖지 않는다.**
#   이 스크립트는 여전히 유효하다: 러닝 인스턴스(2026-08-05 기동분)는 아직 minimal 이고,
#   user_data 는 최초 부팅에만 실행되므로 기존 호스트 복구는 계속 이 스크립트 몫이다.
#   배경 = docs/backlog/ami-filter-picks-minimal-no-ssm-agent.md
#
