#!/usr/bin/env bash
# 프론트 배포 — 로컬 빌드 이미지를 Docker Hub 에 push 후 프로덕션 호스트에서 교체
#
#   bash docs/handoff/scripts/deploy-front.sh [태그]
#
# 전제: docker buildx build --platform linux/amd64 -t mmt2024/mmt-front:<태그> --load .
# ⚠️ 맥(arm64) ≠ EC2(x86_64) — amd64 가 아니면 EC2 에서 exec format error 로 죽는다([1/6] 이 막는다).
# 롤백 = 이미지 태그 되돌리기(ADR-0011) — 마지막에 명령을 찍어준다.
#
# ── 매 배포마다 고쳐 쓰는 값 (아래 3줄) ─────────────────────────────
#   TAG    이번에 올릴 태그 · PREV 롤백 대상(직전 라이브 태그)
#   MARKER 이번 변경에만 있는 번들 내 고유 문자열. [2/6](배포 전)·[6/6](라이브)에서
#          같은 문자열을 찾아 "구 소스로 빌드" · "캐시/구 이미지"를 걸러낸다.
#          갱신을 잊으면 검증이 통과해도 의미가 없다 — 태그와 함께 반드시 바꾼다.
#   HOST   EIP. 인프라 재런치 때 바뀐다.
# 마지막 실사용: 2.0.2 (2026-08-15, MARKER="몇 번째 계단인지" — 큐 DAG 그래프)
# ────────────────────────────────────────────────────────────────
TAG=${1:-2.0.2}
PREV=mmt2024/mmt-front:2.0.1
MARKER="몇 번째 계단인지"
...
# ---------- 2. 수정이 실제로 번들에 들어갔나 ----------
echo
echo "[2/6] 이 빌드에 이번 변경이 들어있나 (배포 전 확인) — MARKER=\"$MARKER\""
HIT=$(docker run --rm --entrypoint sh "$IMAGE" -c "grep -ho \"$MARKER\" /usr/share/nginx/html/assets/*.js 2>/dev/null | head -1")
[ -n "$HIT" ] && ok "번들에 '$MARKER' 존재 (이번 변경)" || bad "번들에 없음 — 구 소스로 빌드됐거나 MARKER 를 안 고쳤다"
...
# ---------- 6. 배포된 번들에 수정이 실렸나 ----------
echo
echo "[6/6] 라이브 번들에 이번 변경이 실렸나"
BUNDLE=$(curl -s --max-time 15 "https://$DOMAIN/" | grep -oE '/assets/[^"]+\.js' | head -1)
echo "      번들: $BUNDLE"
if curl -s --max-time 30 "https://$DOMAIN$BUNDLE" | grep -q "$MARKER"; then
  ok "라이브 번들에 '$MARKER' 존재 — 수정 반영 확인 ✨"
else
