...
- **상태:** ✅ **해소 완료(2026-08-05 재런치) — 이 문서는 이력 아카이브.** 결함 ①② 둘 다 처리됐고
  라이브에서 익명 진단 플로우 완주 검증(Playwright 실브라우저, API 23건 200 · 콘솔 에러 0).
  프로덕션 백엔드 = `mmt2024/mmt-backend:ea94a1a…`(마스킹 수정·하드닝 포함 이미지).
- **인증 문제가 아니었다.** ① M7 진단 테이블이 프로덕션 RDS 에 미적용(→ 500) + ② `/error` 마스킹이
