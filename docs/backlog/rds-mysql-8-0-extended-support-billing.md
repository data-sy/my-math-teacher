...
- **자동 백업이 꺼져 있었다** — `BackupRetentionPeriod = 0`, `skip_final_snapshot = true`.
  업그레이드 직전까지 유일한 복원점이 7/31 스냅샷(= M7 DDL 이전, 8월 데이터 없음)이었다.
  PITR 이 없는 상태라 수동 스냅샷이 선택이 아니라 필수였다. → **별도 항목으로 남길 것**(아래).
...
- **백업 보존 기간을 0 에서 올릴 것인가** — 비용 대 복원 가능성. 위 "부수 발견" 참조.
