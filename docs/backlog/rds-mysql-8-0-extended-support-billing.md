...
- **`engine_version` 표기 diff 를 어떻게 닫을 것인가** (2026-08-31 추가). state `8.4.11` vs
  config `8.4` 라 `terraform plan` 에 in-place 1건이 상시로 뜬다. AMI 필터 지뢰가 제거된 지금
  **이게 plan 에 남은 유일한 항목**이라, 닫으면 plan 이 완전히 clean 해진다(= 다음 인프라 작업의
  diff 신호가 깨끗해진다). 선택지는 두 갈래다 — (a) apply 로 흡수: 버전 관점 no-op 이지만
  RDS modify 라 사람이 실행해야 하고 재시작 위험을 따져야 한다, (b) config 를 `8.4.11` 로 고정:
  apply 없이 clean 해지지만 마이너 자동 업그레이드 때마다 같은 diff 가 되살아난다. **무해하므로
  급하지 않다.**
