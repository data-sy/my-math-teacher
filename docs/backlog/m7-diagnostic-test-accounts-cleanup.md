...
- **상태:** ✅ **완료 (2026-08-15)** — `zdbg-cleanup.sh --delete` 실행. 사후 조회에서 **zdbg 계정 0개** 확인(재조회 2026-08-15). 이 백로그는 닫힌다
- **트리거 시점:** 프로덕션 인프라 재런치 후 → **충족됨.** 프로덕션이 라이브(`mmt-front:2.0.2`)라
  이 계정들이 실제로 살아 있다. 미처리 상태.
- **관련:** [A4 근본원인 백로그](m7-prod-auth-fresh-token-401.md)(이 계정들이 생성된 맥락) · 재런치 절차 = [`🤖-M7-인프라-티어다운-재런치.md`](../handoff/🤖-M7-인프라-티어다운-재런치.md) §3
