...
- `ignore_changes = [ami]` 때문에 **AMI 는 고정된다.** 올릴 때는 사람이 lifecycle 블록을
  일시 제거하거나 taint 로 의도적으로 교체하고, 그때는 호스트 로컬 자산 재구성(재런치 절차 §3)이
  따라온다는 걸 전제로 계획해야 한다.

  > ⚠️ **정정 (2026-08-31):** 이걸 처음엔 "보안 업데이트가 자동으로 따라오지 않는다"고 적었는데
  > **오해를 부르는 서술이라 고쳤다.** terraform 에서 AMI 가 바뀌면 in-place 패치가 아니라
  > **인스턴스 교체**이고, apply 를 자동으로 도는 것도 없다(CI 에 terraform 스텝 0) —
  > `most_recent = true` 는 plan 에 diff 를 만들 뿐 아무것도 적용하지 않았다.
  > 즉 **잃은 자동 패치 경로란 애초에 없었고**, 이 고정으로 실질 보안 태세는 변하지 않았다.
  > AMI 신선도는 **런치 시점**에만 의미가 있다(새 인스턴스가 더 최신에서 출발).
  > 러닝 호스트의 실제 패치는 호스트의 `dnf` 몫이고, 그쪽에 진짜 구멍이 있었다 →
  > [호스트 OS 패치](host-os-patching-al2023-releasever-pin.md)
