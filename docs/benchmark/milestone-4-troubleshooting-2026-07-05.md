...
> 실측 수치는 짝 문서 `milestone-4-run-report.md`, 상태·다음 계획의 정본은 `docs/specs/m4/m4-worklog.md`.

> **[갱신 2026-07-05] 결함 2(Redis 캐시 역직렬화)는 이후 수정 완료.** 근본원인·최종
> 수정·**코드리뷰가 잡은 배포 오버랩 함정**·사람용 교훈은 포스트모템으로 분리:
> `docs/reports/redis-cache-cross-instance-postmortem.md`. 아래 "5. 다음 세션 할 일"의
> 1번(캐시 수정)은 그 문서로 닫혔다. §4 After 재측정만 AWS 게이트로 남음.
