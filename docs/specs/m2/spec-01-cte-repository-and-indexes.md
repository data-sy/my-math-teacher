...
시드 파일: 신규 `api/sql/cte_test_seed.sql` (격리된 소규모 시드, 10-20 row 수준)
...
- **스키마 부트스트랩**: `@JdbcTest`는 JPA 엔티티 스캔을 하지 않으므로 `application-test.yml`의 `ddl-auto: create-drop`이 작동하지 않음. → `@Sql(scripts="classpath:cte_test_schema.sql")`로 명시적 스키마 적용 필요. 스키마는 `create.sql`의 `chapters` + `concepts` + `knowledge_space` 부분만 추출.
- **시드**: 신규 `cte_test_seed.sql` (의도된 케이스 핀포인트, Task 1.5 표 참조).
- **적용**: `@JdbcTest + @Import(TestcontainersConfig.class) + @Sql(scripts={"classpath:cte_test_schema.sql", "classpath:cte_test_seed.sql"})`
- 스키마 파일은 spec-03 회귀 테스트(`@Sql` prod 시드 재활용)와 공유 가능.
...
- [ ] 신규 `api/sql/cte_test_seed.sql` 작성 (2-cycle 케이스 포함)
