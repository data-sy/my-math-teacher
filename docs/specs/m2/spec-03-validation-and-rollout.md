...
**예상 소요:** 3일 (검증)
**선행 spec:** spec-02
**후속:** Phase 5 (점진 출시·관찰·폐기) 는 **M3 (그래프 인프라 폐기 + 운영 출시 정책)** 로 이관됨. 본 spec 의 §Task 5.1 / §Task 5.2 / §Task 5.3 / §완료 기준 의 점진 출시·폐기 항목은 M3 spec 으로 인계되며, 본 문서에서는 M2 검증 단계의 컨텍스트 자료로만 유지된다.
...
> 신규 클래스 `api/src/test/java/com/mmt/api/repository/concept/GraphRegressionSnapshotTest.java` 에 작성한다. `ConceptServiceFeatureFlagTest`는 라우팅 검증 책임을 그대로 유지하고, 본 클래스는 스냅샷 vs CTE 결과 동등성 검증에 한정한다. 스냅샷의 전 키(5 conceptId × depth 2/3/5 = 15 케이스)를 `@ParameterizedTest`로 순회.
...
void cteUniqueNodeSetMatchesNeo4jSnapshot() {
    ...
    var snapshot = loadSnapshotEntry(
        "shared/benchmark/neo4j-snapshot-20260512.json",
...
검증된 사실 (audit 결과):
- ✓ 스냅샷 위치/명명: `shared/benchmark/neo4j-snapshot-20260512.json` (spec-02 작업 후 재생성된 최신본, 단일 파일·날짜 stamp 포함). JSON 최상위 키 = `"conceptId=N,depth=D"`, 값 = `{count, concept_ids[], sha256}`. **`concept_ids`는 multiset (UNWIND 결과 중복 포함), `count`는 multiset 길이**. M1 산출본 `20260424.json`은 동일 키 구조이며 `shared/benchmark/` 에 회귀 참조 자료로 함께 보관(재마이그레이션 대비).
...
본 채택은 **ADR 0006으로 기록**한다(`docs/adr/0006-m2-gradual-rollout-policy.md`). ADR 작성 사유: 향후 다른 마이그레이션·기능 출시에서 동일 단일 인스턴스 조건이라면 (C) 정책을 재사용 가능하므로 일회성 결정이 아니라 정책 기록.
...
- [ ] `.block()` 잔존 호출 제거 (spec-02 Task 3.2 후속) — 현행 위치: `KnowledgeSpaceService.java:39`, `ConceptService.java:164`. `ProbabilityService.java:66`은 spec-02 Task 3.4 마이그레이션으로 이미 해소됨
...
- [ ] `docker-compose.yml`에서 `mmt-neo4j` 컨테이너 제거 — **ADR 0007 통합본 동반** (루트 CLAUDE.md "docker-compose.yml의 서비스 구성은 ADR 없이 변경 금지")
...
- [ ] Task 4.1~4.4 모두 통과
- [ ] Task 5.1 채택안 (C) 실행 완료 + ADR 0006 기록
- [ ] 관찰 기간 무사고 (기본 1개월, [검증 필요] 단축/연장 기준)
- [ ] Task 5.3 체크리스트 모두 완료
- [ ] ADR 0007 통합본 작성 (Neo4j 도입·폐기·docker-compose 변경)
...
- M1 산출물: `docs/benchmark/milestone-1-baseline.md`, `shared/benchmark/neo4j-snapshot-20260512.json` (회귀 기준), `shared/benchmark/neo4j-snapshot-20260424.json` (M1 원본, 보관)
