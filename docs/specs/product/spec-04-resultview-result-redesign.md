...
3. **progressive disclosure** — 기존 표(`DataTable`)·그래프 트리·개념 상세를 "근거 더보기"로 강등(접힘). 기능은 보존, 재사용.
...
### 4.4 progressive disclosure (강등)

상단(요약+카드) 아래, 기존 상세는 **"근거 더보기"** 토글(Accordion/Panel, 기본 접힘) 안으로 이동 — **기능·컴포넌트 보존, 위치만 강등**:
- 문항별 표(`DataTable`, L366~401) — 잔여 내부용어 카피 점검 후 그대로.
- 선수지식 트리(`useConceptGraph` 그래프 + 범례, L405~437) — 동작 불변.
- 개념 상세보기 1·2(L439~521) — 동작 불변(빈 상태만 D4 에서 개선).

카드의 "선수지식 트리 보기" → 기존 `showTree(representative.conceptId)`(L198~250) 호출 시 "근거 더보기"를 펼치고 해당 트리로 스크롤(기존 `scrollIntoView` 재사용).
...
- **Task 3 — progressive disclosure.** 기존 표·그래프·상세를 "근거 더보기"(기본 접힘)로 강등, 동작 보존. 카드→트리 스크롤 연동.
