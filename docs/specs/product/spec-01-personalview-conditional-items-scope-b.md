...
### D3. 문항 수(count)
- `count` 는 **D1 기반 신규 문항의 상한(LIMIT)**. 범위 6~30, 서버에서 clamp.
- 재출제 원본 문항은 count 와 **무관하게 그 위에 추가**(레버 독립·예측 가능). 따라서 최종 학습지 = (재출제 원본 문항) + (신규 문항 ≤ count).
- 신규 문항 후보(필터된 distinct conceptId) 가 count 보다 적으면 있는 만큼만(폴백 없음).

### D4. 정렬·중복 제거
- 학습지 순서: **재출제 원본 문항 먼저 → 신규 문항**. `testItemNumber` 는 최종 합쳐진 순서대로 1부터.
- 중복 제거: `item_id` 기준. 재출제 원본과 신규 문항이 같은 item 이면 1회만(원본 우선).
- 신규 문항 후보는 distinct `concept_id` 로 축약(레거시는 미축약이나 신규 경로는 count 의미를 위해 distinct).
...
1. **T1 — 리포지토리**: 원본 응시 문항 조회 추가. `ItemRepository.findOriginalItemsByUserTestId(Long userTestId, boolean onlyWrong)` → `List<Item>` (answers⋈items⋈concepts⋈chapters, onlyWrong 시 `answer_code=0`). 단일 쿼리(N+1 없음).
...
- [ ] `count` 캡 동작(신규 문항 ≤ count), 6~30 clamp
