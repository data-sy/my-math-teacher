...
### D1. 맞춤 유형 → 우선순위 tier (위주 = 우선순위, 배타 아님; 개정 2026-06-23)
"위주"는 배타 필터가 아니라 **우선순위**다. 해당 depth 를 **먼저** 채우고, count 가 모자라면 나머지 depth(≤2)로 **넘어가(spill)** 채운다.

| 라디오 값 | primary(먼저) | secondary(부족 시) |
|---|---|---|
| `wrong` (오답 문항 위주) | `to_concept_depth == 0` | `1 <= depth <= 2` |
| `prerequisite` (선수 지식 위주) | `1 <= depth <= 2` | `depth == 0` |

- 출제 범위 = 학생의 `depth ≤ 2` 개념 **전체(union)**. depth 3 제외.
- 한 범주(예: 오답=depth0)가 작아도 다른 범주(선수지식)로 넘어가 count 를 채운다 → "위주"만으로 count 미달되는 문제 해소(사용자 결정 2026-06-23, "위주=우선순위").
- 정밀한 비율 배분(예 7:3)은 아래 §향후 개선(별도 후속).
...
- 채우기 순서:
  ...
  3. 원본 수 < count → 남은 자리(`count − 원본수`)를 **D1 우선순위 tier 순서**로 채운다:
     - (a) primary tier(맞춤유형 우선 depth) 개념들에서 round-robin(개념당 1문항씩 → 추가 패스),
     - (b) 아직 자리가 남으면 secondary tier(나머지 depth≤2) 로 **넘어가(spill)** 동일하게 round-robin,
     - (c) primary+secondary 전체(depth≤2) 가용 문항이 소진되면 중단(count 미달 가능 — 없는 문항 생성 안 함).
- 이전 안(“count=신규 상한, 재출제 additive on top”)·D1 배타 필터는 폐기. 사용자 의도 = 내가 정한 문항 수가 곧 학습지 크기, "위주"는 우선순위.
