...
|---|---|---|---|
| `/` | ① 홈 | 익명 | 1차 CTA "무료 진단 시작" 단독 + 그래프 히어로(탭 무동작 미끼) + "둘러보기" 텍스트 링크. **조건부 재진입 배너**(로그인+활성 큐 시에만 `GET /learning-queues/me` 1회 — 비로그인 무호출): "이어서: {첫 미완료 개념} ›" → ④-B 직행, 전 항목 완료면 **문구만 대체·탭 동일**(A안). **조용한 인증 상태 행**(상시 노출 — 비로그인 = "로그인" → ⑤-A 변형(귀속 생략, 성공 시 홈 복귀) / 로그인 = "로그아웃" → `DELETE /auth/authentication` + 로컬 토큰·큐 캐시 제거). 배너와 **배타 아님**(2026-08-06 갱신 — 구 배타 규약은 "로그인 + 큐 없음"에서 이 자리를 통째로 비웠고 로그아웃 진입점이 어디에도 없었다. 사유 = `01-home.html` ●7) |
