...
- 게이트 통과(로그인) 후 `POST /diagnosis` 재제출 성공 시 **삭제**(정본이 서버로 이동). preview 와 귀속 결과의 동일성은 spec-01 결정론 계약이 보장.
...
- **게이트 결정론**: preview 결과와 귀속 후 결과 화면이 동일(spec-01 §8 연동 — 프론트에선 스냅샷 비교).
