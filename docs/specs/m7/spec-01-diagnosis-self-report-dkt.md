...
**상태:** 📝 **spec 합의 대기** → §7 결정(S1~S6) 사인오프 후 코드 단계(`/analyze-before-change` → 구현)
...
3. **self-report→DKT 재배선** — answered-map 을 정오답 시퀀스로 매핑해 `findAIInput`→TF Serving→`probabilityList` 경로 재사용, `probabilities` 확정 저장.
...
- **`items` 풀 정리** — self-report 는 `items` 미사용 → 유휴화만(단 S2-B 선택 시 대표 item 참조가 남음 — §7).
...
| answers 스키마 | `answers(answer_id, user_test_id, item_id→tests_items FK, answer_code)` (create.sql:138-146). ⚠️ 구 spec 은 FK 제약 미인지 | **S2 결정** — concept 단위 저장을 위한 스키마 선택 필요 |
| DKT 입력 빌드 | `AnswerService.findAIInput`: `findBefore`(동일 유저 전체 이력) → utId 별 `findAnswerCode`(items→concepts 조인으로 `skill_id` 도출) → `[skillId, answerCode]` 쌍 **×10 증폭** (AnswerService.java:29-47, AnswerConverter.java:32-34) | **재배선** — answered-map 에서 직접 시퀀스 생성. 이력 포함 여부 = **S3** |
...
| 저장 스키마 | `probabilities(probability_id, answer_id, concept_id, to_concept_depth, probability_percent, probability_timestamp)` — skill_id 컬럼 없음 | **무변경 재사용** |
...
**핵심:** DKT 결합 3곳(`findAIInput`·`getPrediction`·`probabilityList[skillId-1]`)과 `probabilities` 골격을 **그대로 살리고**, 입력 진입점(제출 DTO→answers)과 출력 계약(결과·큐)만 갈아끼운다.
...
게이트: POST /api/v1/diagnosis {entry, answered[]} (인증)          ← 귀속 저장: users_tests+answers+probabilities
...
1. answered-map → **안다=1(맞음) / 모른다=0(틀림)** 으로 `answer_code` 매핑, `users_tests` 1행 + `answers` N행 저장(S2 스키마 결정 반영).
2. DKT 입력 = answered-map 에서 `[skillId, answerCode]` 시퀀스 직접 생성(`skill_id = concepts.skill_id`, items 조인 불요) + **×10 증폭 유지**(입력 최소 크기 3 안정 통과 — 현행 근거 승계). 이력 포함 여부 = S3.
3. `getPrediction` 재사용 → `probabilityList`. **가드 신설**: TF Serving null 응답, `skill_id = -1`(미매핑 개념), `skillId-1` 범위 초과 — 셋 다 현행엔 없음(§3).
4. 취약 확장 재사용: depth0 = "몰라요" 개념들 → `findPrerequisitesAsDepthMap(cId, 3)` → `probabilities` 저장. `probability_percent = probabilityList[skillId-1]` 인덱싱 유지.

**결과 계약** — `POST /diagnosis/preview` 응답 = `GET /api/v1/diagnosis/{userTestId}` 응답 (동일 shape, 후자만 영속 조회):
...
- 카드 = **개념 단위 dedup**(probabilities 는 answer_id 별 중복 행 허용 — 조회 시 개념별 대표값 집계: `MIN(to_concept_depth)`, percent 는 skill 단위라 동일).
...
- 구 경로(`/weakness-diagnosis`·`TestController`)는 무변경 — 롤백 = 플래그 off + 구 Vue 프론트(신·구 병행 가능 구조, 피처 플래그 정책 충족).
...
- **S2 — self-report 답안 저장 스키마:** **A(권장) `answers.concept_id` nullable 컬럼 추가**(additive, item_id 는 self-report 행에서 NULL 허용으로 완화 — FK 제약 변경 포함, analyze-before-change 대상) — 의미 정직·items 유휴화와 정합 / B 개념당 대표 item 매핑 — 스키마 무변경이나 유휴화 예정인 `items` 에 새 결합을 만들고 의미 왜곡(가라 이미지 문항이 self-report 근거로 남음).
- **S3 — DKT 입력 구성:** **A(권장) 현재 세션 answered-map 만** ×10 — 익명 preview 와 귀속 결과의 결정론 동치 보장, 스냅샷 설명가능 / B 계정 이력 포함(`findBefore` 승계) — 시퀀스 풍부하나 preview(이력 없음)와 귀속(이력 있음) 결과가 달라져 F-1 "본 결과를 저장" 기대 위반.
...
- **items 무의존:** self-report 경로가 `items`·`tests_items` 를 참조하지 않고 완주(S2-A 시).
