# MMT 프론트 v2 — 얇은 프론트 스펙 (구현 정본)

> 정본 기획물(플로우 맵 + 와이어프레임 6종 + PRD + 접점 시트)에 **없는 기술 결정만** 기록한다.
> 기획물 재서술 금지 — 화면·인터랙션·API 계약의 정본은 `docs/design/v2/` 그대로.
> 작성 2026-07-18(제로베이스 샌드박스) · **이 리포 정착 2026-07-18** · 가정 로그 = [`assumptions.md`](assumptions.md).
>
> **[spec-02-react-mvp.md](spec-02-react-mvp.md) 와의 관계:** spec-02 는 초안(사인오프 대기 T1~T5)이었고, 제로베이스 실빌드 + 사용자 확정(🤖 빌드 문서 "재론 금지 결정")으로 T1~T5 가 재확정됐다 — T2 = Context(Zustand 아님)·T3 = CSS Modules(Tailwind 아님)·T5 = `web-v2/`(web-react 아님). 구현 결정의 정본은 본 문서, 백엔드 접점 사실·에러 카탈로그·검증 항목은 spec-02 §2·§4.6·§6 이 여전히 유효.
...
| 라우팅 | **react-router v7** (`createBrowserRouter`) | SPA fallback 전제와 일치하는 history 라우팅. 화면 6개 + 횡단 2개 규모에 표준 선택. *(샌드박스 스펙 초안의 "v6" 표기는 오기 — 실설치 `react-router-dom ^7.18.1`, 재론 금지 결정도 v7.)* |
...
| 스타일링 | **CSS Modules (plain CSS)** | 디자인 토큰은 `:root` CSS 변수 한 벌이면 충분(§7 틸 그로스). Tailwind/CSS-in-JS는 의존성 무게 대비 이득 없음. |
...
| mock | **MSW (Mock Service Worker)** | fetch 레이어를 그대로 두고 네트워크 경계에서 가로챔 → mock↔실서버 전환이 코드 무변경(핸들러 on/off). `VITE_ENABLE_MOCK=false`면 실서버로 통과. 개발 기본값 = mock on. |
| 테스트 | **Vitest + Testing Library** (핵심 로직 단위) + **Playwright** (390px 플로우 스모크) | 핵심 플로우(①→②→③→④, 게이트 전환, ⑥ 탐색) 자동화. 전 화면 E2E 매트릭스는 MVP 과체중. |
...
*(spec-02 초안의 `/diagnosis/entry` 류 프리픽스 라우트는 실구현에서 상기 플랫 경로로 확정 — e2e·와이어프레임 링크가 이 경로 기준.)*
...
| 키 | 값 | 근거 |
|---|---|---|
| `mmt.accessToken` | string | 접점 시트: 콜백 `?token=` 파싱 보관. 보관 방식은 설계 자유 → localStorage 채택(A-3). *(구 Vue 의 `accessToken` 키와 별개 — 앱 분리.)* |
| `mmt.gradeSemester` | `{grade, semester}` | 02 검수 확정: 학년 소스 = 최초 방문 선택 → localStorage 기억. 서버 저장 없음. |
| `mmt.diagSession` | `{entry, chapterId, chapterName, answered: [{conceptId, know}], done: boolean, savedAt}` | F-2 answered-map. 매 답변 직후 저장. `done=false`만 ② 이어가기 대상(S1-A). `done=true`는 ④ 체류 중 새로고침 복원용으로만 유지(●1: 이탈 시 소실 허용). |
...
- `answered[]` 는 **append-only·순서 보존** — spec-01 §4.4-2 시퀀스 순서 조건(DKT 순서 민감 → preview==귀속 결정론)의 프론트 측 필요조건. undo = 말단 pop 만.
...
web-v2/src/
  ...
  styles/     (토큰은 index.css :root — §7) · 화면별 *.module.css
```
...
- **mutation 실패도 무음 금지(2026-07-18 POC 발견):** ④-B 체크리스트 done 토글 등 쓰기 실패 시 인라인/토스트 피드백 필수 — onError 무처리로 "탭했는데 아무 일도 없음"을 만들지 않는다.

## 6. 이 스펙의 가정

개별 가정은 [`assumptions.md`](assumptions.md)에 기록(무엇을·근거·뒤집는 법). 스펙 레벨 요약: 토큰 보관 = localStorage(A-3), mock OAuth = 가짜 콜백(A-4), 시기 추정 진도표 = mock 데이터 내 하드코딩 표(A-5 — R6 확정대로 soft 제안일 뿐).

## 7. 디자인 토큰 — 하이파이 "틸 그로스" (2026-07-18 확정)

방향 결정 = 블라인드 전문가 평가(격리 세션·순서 카운터밸런스) + 사용자 확정. 평가 기준 = 인터넷 공개 시 클릭 유발·유입 확보. 근거 아카이브 = 재료 창고(제로베이스 리포) `docs/design/eval-results/`.
...
- 적용 원칙: 팔레트 스왑만(레이아웃·보더 두께·위계 구조 불변). ③ 2버튼 = 동급 무게 + 색 분화(03 ●4 확정).

## 8. 실서버 검증 예정 표 (실계약 대조 — 3·5단계에서 가정→사실 갱신)
