# 가정 로그 — 프론트 v2 (제로베이스 빌드 → 이 리포 정착 2026-07-18)

> 형식: 무엇을 가정했나 · 근거 · 뒤집으려면 어디를 고치나.
> 기획물이 침묵하는 지점만 — 기획물과 모순되는 가정 금지.
> 실서버 검증 대상·상태 추적은 [`frontend-spec.md`](frontend-spec.md) §8 표가 정본 — 검증 완료 시 이 파일의 해당 항목을 가정→사실로 갱신한다.
> A-1 은 샌드박스 환경(백엔드 무접근) 서술 — 이 리포에선 로컬 백엔드 기동 가능하므로 실연동 검증(3단계)으로 소화된다.
...
## 실서버 검증 결과 (2026-07-18 — 상세 = frontend-spec.md §8)

- **A-2 → 사실(정정 포함):** next/preview shape 일치. 단 `Answer` 필드는 `known`(`know` 는 서버가 조용히 무시 — 알아요가 전부 몰라요로 둔갑하는 파손이라 전면 정정). 에러 바디는 `{message}` JSON 이 아니라 **plain text**(401 은 빈 바디) — client 텍스트 폴백으로 흡수.
- **A-9 → 뒤집힘:** `GET /concepts?chapterId` 는 이름 목록만. 그래프 = `nodes/{id}`+`edges/{id}` **중심 개념 서브그래프**(전체 그래프 엔드포인트 없음 — '모두 보기'는 선택 중심 이웃으로 동작, 백로그 후보). edge source→target = 선수→후수.
- **A-10 → 뒤집힘:** 통짜 detail 없음 — `/{id}`+`prerequisite/{id}`(자기 자신 포함)+nodes+edges 4콜 조립. search 는 `conceptChapterName` 등 별도 필드(chapterId 없음 — 단원명 매칭).
- **A-11 → 정정:** items 필드 = `queueItemId` + 서버 파생 `current`. 토글 응답 = 큐 전체. ⚠️ 엣지: 알아요 폐쇄가 후보를 전부 거르면 **빈 큐** 생성(카드는 약점인데 계단 0 — 백엔드 병합 로직 재검토 백로그).
- **A-13 → 뒤집힘:** chapters 실응답 = 구 Vue 트리 shape·(학년,학기) 한글값 단위 조회 → 중등 6개 학기 fan-out + leaf(=chapter_id) 평탄화. 고등 커리큘럼(수학·수1·상/하/전체)은 학년·학기 도메인과 미매핑 — 백로그.
- **A-1 → 소화:** 이 리포에선 백엔드 로컬 기동 + 진단 플래그 on 으로 실연동 완료(preview==귀속 결정론 동치 확인).
