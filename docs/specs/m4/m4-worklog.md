...
- **Terraform = spec-03 으로 승격(백로그→실행 스펙).** D1~D3 잠금: **D1 LocalStack**, **D2 `infra/terraform/`**, **D3 local state + provider 버전 핀**. 재논의 금지. 출처 backlog 문서는 **2026-08-06 흡수·폐기** — 배경·경계(학습 chosen 경로 · Phase A→B→C · "무과금의 선 = apply 를 안 누르는 것")는 spec-03 머리말과 ROADMAP `[Infra] Terraform` 항목으로 이관.
