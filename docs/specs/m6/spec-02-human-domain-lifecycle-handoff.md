...
> ⚠️ **상태: 방식 확정 — 실행값 착수 시 채움.** spec-01 §7 결정 사인오프됨(2026-07-11): D1=TF Serving 유지,
> **D2=Let's Encrypt+certbot**, D3=RDS(관리형이라 별도 사람 작업 없음). 아래 각 게이트의 방식은 확정, 남은 `[TODO]`
> 는 착수 시점의 **구체 값**(EIP·임계값·만료일)뿐이다. 여기 담기는 건 **어시스턴트가 대신 못 하는 비가역·사람 전용 작업**뿐이다.
...
### G4 — TLS 인증서 (사람 개입 지점) — **D2=(a) Let's Encrypt 확정**
- [ ] certbot 으로 `www.my-math-teacher.com` 발급(nginx 443 종단) + 자동갱신 타이머 확인 (ACM 미채택)
