...
> **비밀번호를 바꿀 때는 두 파일만 고쳐선 안 된다.** compose 의 `MYSQL_PASSWORD` 는 **볼륨 초기화 때만** 적용된다.
> 기존 `mysql-vol` 이 있는 상태에서 값만 바꾸면 컨테이너는 새 값으로 뜬 척하고 실제 인증은 옛 값으로 계속된다(조용한 위장 성공).
> 실제로 바꾸려면 컨테이너 안에서 `ALTER USER 'mmt2024'@'%' IDENTIFIED BY '<새값>'` 을 치거나 볼륨을 새로 만든다(후자는 시드 재적재 동반).
> 바꾼 뒤 검증은 **새 값 접속 성공 + 옛 값 거부** 를 둘 다 확인한다 — 앞의 하나만 보면 위장 성공을 못 걸러낸다.
