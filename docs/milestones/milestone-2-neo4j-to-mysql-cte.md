...
| 캐싱 레이어 신규 도입 | 낮음 | 낮음 | RedisUtil 직접 호출 채택(ADR 0004)으로 영향 최소화 — Spring Cache 인프라 미도입 |
...
- ADR 0003: knowledge_space 엣지 방향성 (CTE는 backward 단일안)
- ADR 0004: M2 캐싱 패턴 (RedisUtil 직접 호출, Spring Cache 미도입)
- ADR 0005: M2 모니터링 인프라는 M3로 분리, M2는 SimpleMeterRegistry 검증 단계만
