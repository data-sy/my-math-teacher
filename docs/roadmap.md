...
- **[Product] 맞춤학습지 조건부 출제 (Scope B)** — 구현·단위테스트·프론트 연동·**런타임 검증 통과**, PR 대기 (브랜치 `feat/personalview-conditional-items-scope-b`, 2026-06-23)
  - 런타임 검증(2026-06-23): 풀스택 기동 후 8개 시나리오(depth 필터·재출제 additive·item_id dedup·순서·count clamp·하위호환·IDOR 401) 전부 기대값 일치. 백엔드 단위테스트 13종 통과.
  - PersonalView 의 맞춤 유형(오답 위주/선수 지식 위주)·재출제(없음/오답/전체)·문항수 라디오가 백엔드 `ItemService.findPersonalItems` 를 제어하도록 구현. (Scope A 기본 정책은 PR #24 `5623ffc` 출시 완료.)
  - **그래프 무의존 확인**: `findPersonalItems` 는 라이브 그래프 탐색이 아니라 `probabilities.to_concept_depth`(사전계산) 만 필터링한다 → Neo4j/CTE(`mmt.migration.use-mysql-cte-for-graph`) 직접 의존 0. 따라서 로드맵 원안의 "M3 출시 후" 전제는 **비차단**(M3 미출시여도 착수 가능, 사용자 결정 2026-06-23).
  - 결정: 맞춤유형="위주"=우선순위 tier(오답위주: depth0 먼저→depth1~2 spill / 선수지식위주: depth1~2 먼저→depth0 spill, D1 개정 2026-06-23), 재출제=원본 응시문항 재포함, **count=목표 총 문항수**(6~30 clamp) — 원본 → tier 순서 round-robin 으로 count 까지 채움(depth≤2 전체 소진 시 미달 허용), 파라미터 옵셔널=레거시 하위호환.
  - spec: [`docs/specs/product/spec-01-personalview-conditional-items-scope-b.md`](specs/product/spec-01-personalview-conditional-items-scope-b.md). 남은 일: 브라우저 UI 검증 → PR.
  - **후속(deferred)**: 비율 배분 출제(선수지식:일반지식 ≈ 7:3 자리 배분, 오답을 일반지식 자리로 상한) — 현재는 우회안(원본 먼저+범주 채움)으로 출시, 비율 배분은 아래 Later 백로그.
- **[Product] 맞춤학습지 비율 배분 출제** (Scope B 후속, 2026-06-23) — count 를 맞춤유형/재출제 비율(예 선수지식:일반 7:3)로 자리 배분하고 버킷별 상한·보충. Scope B spec §향후 개선 참조.
- **[Design] 실배포 전 리디자인** (전문가 페르소나 컨설팅 2건 수렴, 2026-06-23)
  ...
  - 이월 UI 항목:
    ...
    - `listboxTest` 무가드 역참조 잔여 (DiagView 다운로드 다이얼로그 ~355·371행) — PersonalView 는 `d15f6aa` 로 옵셔널 체이닝 처리, DiagView 도 동일 처리 권장.
