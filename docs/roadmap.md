...
- **[Design] ResultView 선수지식 트리 "누적해서 보기" affordance** (spec-04 D5 에서 이월, 2026-06-24)
  ...
  - **주의(2026-06-25):** affordance 자체는 UI-only 라 데이터 비의존 → 토큰 sweep+리스킨 묶음 PR 에 동반 진행 예정(핸드오프 ▶▶ 참조). 아래 임계 보정·정답률 차트와 달리 실배포 전 가능.
- **[Design] ResultView 시급도 임계 도메인 보정 + 정답률 차트** (spec-04 B-3 에서 분리, **실배포 후·데이터 의존**, 2026-06-25)
  - spec-04 B-3 에서 시급도 뱃지 임계를 절대구간 **40/65%** 로 잡았으나(상대 thirds 대체) 이는 제안 출발값 — 실유저 mastery 분포가 쌓여야 도메인 보정이 의미 있다. 데이터 없이 만지면 추측이라 **실배포 후 실사용 데이터 확보 시점에 보정**한다.
  - 정답률 게이지/차트도 동일: weakness-diagnosis 응답에 정답수·점수가 없어 데이터 확보 후 진입(spec-04 D2 에서 후속으로 분리해 둠). Chart.js 이미 도입돼 있음.
  - **개념 단위 약점 집계**(현재 문항 단위 카드)도 이 트랙에 함께 검토. 전제: depth-0 대표문항 시드 보강([Data] 샘플 누락 항목)·실유저 데이터.
