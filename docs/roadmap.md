...
- **[Design] 실배포 전 리디자인** (전문가 페르소나 컨설팅 2건 수렴, 2026-06-23)
  ...
  - 이월 UI 항목:
    ...
    - ~~`listboxTest` 무가드 역참조 잔여 (DiagView 다운로드 다이얼로그 ~355·371행)~~ — ✅ `fa991d0` 처리 완료(브랜치 `fix/diagview-listboxtest-null-deref`). PersonalView `d15f6aa` 와 동일 패턴: watch 해제 시 게이트 초기화 + 두 Dialog 옵셔널 체이닝. 런타임 수동검증·PR 대기.
