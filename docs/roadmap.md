...
- **[Product] 맞춤학습지 조건부 출제 (Scope B)** — 구현·단위테스트·프론트 연동·**런타임 검증 통과**, PR 대기 (브랜치 `feat/personalview-conditional-items-scope-b`, 2026-06-23)
  ...
  - 결정: 맞춤유형=depth 필터(오답=0 / 선수지식=1~2), 재출제=원본 응시문항 재포함, **count=목표 총 문항수**(6~30 clamp) — 원본 → 같은 범주 round-robin 으로 count 까지 채움(범위 소진 시 미달 허용, D3 개정 2026-06-23), 파라미터 옵셔널=레거시 하위호환.
