...
- **[Product] 맞춤학습지 조건부 출제 (Scope B)** — 구현·단위테스트·프론트 연동·**런타임 검증 통과**, PR 대기 (브랜치 `feat/personalview-conditional-items-scope-b`, 2026-06-23)
  ...
  - 결정: 맞춤유형=depth 필터(오답=0 / 선수지식=1~2), 재출제=원본 응시문항 additive 재포함, count=신규문항 상한(6~30 clamp), 파라미터 옵셔널=레거시 하위호환.
  - spec: [`docs/specs/product/spec-01-personalview-conditional-items-scope-b.md`](specs/product/spec-01-personalview-conditional-items-scope-b.md). 남은 일: spec §검증 체크리스트(런타임) → PR.
