...
- **[Product] 맞춤학습지 조건부 출제 (Scope B)** (pre-launch cleanup 에서 분리, 2026-06-23)
  - PersonalView 의 맞춤 유형(오답 위주/선수 지식 위주)·재출제(없음/오답/전체) 라디오가 실제로 백엔드 `ItemService.findPersonalItems` 를 제어하도록 구현. 현재는 비활성 플레이스홀더이고 **기본 정책(Scope A: 오답→선수지식 깊이≤2 자동 출제)만 출시**됨 (PR 예정 `feat/personalview-ship-default-policy`).
  - 조건부 출제 = 핵심 비즈니스 로직 + 그래프 탐색 변경 → M2 검증된 CTE 그래프 경로(M3 출시 후) 위에서 설계. `findPersonalItems` 의 probabilities depth 필터를 조건 파라미터화.
